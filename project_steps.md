# StockBallDB â Project Steps

## Step 1 â Commit and push documentation updates
...
## Step 2 â Plan Phase 0 foundation

Inspected the documentation-only repo and produced an approved Phase 0 plan: `src/stockballdb` package, config/logging/DB health check, Alembic scaffold, pytest, README â no schema tables yet.

## Step 3 â Implement Phase 0 project foundation
...
## Step 4 â Catch up project_steps and verify Phase 0
...
## Step 5 â Add phase-summary Cursor rule
...
## Step 6 â Phase 1A trading_days design

Produced and locked the Phase 1A design: `pandas_market_calendars` NYSE authority, keep all 15 fields, ISO week/weekday, NULL prev/next edges, generate/upsert/validate pipeline â no other tables yet.

## Step 7 â Implement Phase 1 trading_days
...
## Step 10 â Phase 2B derived-field definition investigation
...
## Step 11 â Implement Phase 2B derivation
...
## Step 12 â Phase 2C market_outcomes definition investigation

Investigated forward-outcome conventions on live ETF data (close-close vs open-entry, max excursion, horizon completeness, look-ahead). Produced recommendations only â no `market_outcomes` table or migration.

## Step 13 â Implement Phase 2C market_outcomes
...
## Step 14 â Phase 2D asset_regimes definition investigation

Investigated point-in-time `asset_regimes` field definitions on live ETF data (trailing returns, SMAs, vol, drawdown, regime labels). Recommendations only â no table, migration, or data changes.

## Step 15 â Implement Phase 2D asset_regimes
...
## Step 16 â Phase 2E macro_conditions definition investigation

Investigated FRED/ALFRED series IDs, revision/PIT rules, release alignment, PMI reproducibility, credit-spread and regime options using live API metadata. Recommendations only â no `macro_conditions` table or data changes.

## Step 17 â Note stale FRED probe failure notification
...
## Step 18 â Implement Phase 2E macro_conditions
...
## Step 19 â Phase 2F scheduled_events investigation
...
## Step 20 â Implement Phase 2F scheduled_events
...
## Step 21 â Phase 2G calendar_context investigation
...
## Step 22 â Implement Phase 2G calendar_context
...
## Step 23 â Phase 3A V1 integration investigation
...
## Step 24 â Implement Phase 3B V1 integration & reproduction
...
## Step 25 â Confirm background build/validate task results
...
## Step 26 â Stamp StockBallDB V1.0.0

Bumped package/project version to `1.0.0` (`pyproject.toml`, `stockballdb.__version__`); updated README, `phase_history.md` V1 freeze milestone, workflow Â§12 title. pytest 84/84; Git tag `v1.0.0` not created (V1 codebase not yet committed to git).

## Step 27 â Confirm V1.0.0 verification run
...
## Step 28 â Finalize V1.0.0 Git snapshot
...
## Step 31 â Phase 4A macro source & definition lock
...
## Step 32 â Confirm Phase 4A pytest run

Background pytest after Phase 4A doc-only changes: **84/84 PASS**. Phase 4A complete; ready for Phase 4B contract-wise.

## Step 33 â Phase 4B macro_conditions ingestion & validation

Executed `build_macro_conditions` twice (17532 rows, idempotent); coverage/PIT/yield-curve audits via `scripts/phase4b_macro_audit.py`. pytest **84/84**; whole-DB `validate_v1` stale on `calendar_context` (17531 vs 17532 TD) â cross-table spine lag, not macro defect.

## Step 34 â Phase 4B closeout: calendar_context resync

Resynced `calendar_context` via `build_calendar_context` (17532 rows, idempotent Ã2); spine aligned; `validate_v1` **PASS**; pytest **84/84**. Phase 4B formally closed.

## Step 35 â Phase 5A market context source & definition lock
...
## Step 36 â Phase 5B WTI implementation

Applied migration `a8f3c2d1b4e5` (nullable OHLC + row-shape CHECK). Built WTI via FRED `DCOILWTICO` (`build_wti` Ã2, idempotent): **10,201** rows `1986-01-02`â`2026-08-25`; negative `-36.98` on 2020-04-20 preserved. Close-only derive/outcomes/regimes; `validate_v1` PASS; pytest **90/90**; audit script `scripts/phase5b_wti_audit.py`.

## Step 37 â Phase 6A scheduled events source & PIT contract
...
## Step 38 â Confirm Phase 6A pytest baseline

Re-ran full suite after Phase 6A doc updates: **90/90 PASS** (~31s). No code or schema changes.

## Step 39 â Phase 6B scheduled events ingestion & validation

Executed full Phase 6B pipeline: preflight audit, `build_scheduled_events` (**2,641** rows across four locked families), `calendar_context` resync (**17,532** rows), historical coverage audit, spot checks, non-trading-day verification, idempotency (events Ã2, calendar Ã2). Fixed FOMC `fomccalendars.htm` parser (`Apr/May`, asterisk dates, row isolation) in `src/stockballdb/events/fomc.py`; added `scripts/phase6b_scheduled_events_audit.py`. `validate_v1` **PASS**; pytest **90/90 PASS**. Phase 6B **COMPLETE**.

## Step 40 â Phase 7A calendar context audit & definition lock
...
## Step 41 â Phase 7B calendar context verification & closeout

Rebuilt `calendar_context` (**17,532** rows, idempotent Ã2). Created `scripts/phase7b_calendar_context_audit.py`; all 19 Phase 7A invariants PASS. Event coverage cross-check: 37 off-calendar events; 47 multi-family trading days. `validate_v1` **PASS**; pytest **92/92 PASS**. Phase 7 **COMPLETE**; Â§16 verification appended to Phase 7A contract.

## Step 42 â Phase 8A whole-DB integrity & provenance audit

Audited all seven canonical tables (live DB + code/models/validators). Locked Phase 8A contract in `docs/StockBallDB_phase8a_validation_provenance_contract.md`: health definition, validation matrix, coverage/freshness/provenance contracts, cross-table invariants, severity taxonomy, health-report design, Phase 8 vs 9 boundary, Phase 8B backlog. Live audit `scripts/phase8a_health_audit.py` â **HEALTHY** (INFO: ETF/WTI publication lag). `validate_v1` **PASS**; pytest **92/92 PASS**.

## Step 43 â Phase 8B health engine, freshness, gaps & build manifest

Implemented `src/stockballdb/health/` with `python -m stockballdb.health` (human + `--json` + `--strict`); reuses `validate_v1`; coverage/freshness/gap detection; JSON build manifests under `build_reports/`; `build_v1` integration (health summary + manifest on success). Live health **HEALTHY** (3 INFO: ETF lag, WTI lag, WTI provider gaps); `validate_v1` **PASS**; pytest **103/103 PASS**. Phase 8 **COMPLETE**; Â§22 appended to Phase 8A contract.

## Step 44 â External data acquisition path audit
...
## Step 45 â Phase 9A snapshot & exact-rebuild architecture contract
...
## Step 46 â Phase 9B immutable snapshot implementation

Implemented snapshot store (`src/stockballdb/snapshots/`), provider fetchâbytesâsnapshotâparse boundary, manifest schema 1.1, database/table fingerprints, `python -m stockballdb.snapshots verify`, `python -m stockballdb.rebuild_exact`, `build_v1`/`build_wti` snapshot integration. Added `docs/StockBallDB_snapshots_and_rebuilds.md`, gitignored `snapshots/`. pytest **120/120 PASS**; validate_v1 **PASS**; health **HEALTHY**. Live BUILD LATEST + REBUILD EXACT certification pending clean Git + rebuild test DB.

## Step 47 â Phase 9B certification attempt (blocked)
...
## Step 48 â Phase 9B certification resume (Part F)
...
## Step 49 â Phase 9B certification complete
...
## Step 50 â Phase 9 documentation closeout

Reviewed and finalized Phase 9 certification docs (`phase_history.md`, `project_steps.md`, phase 9 contract, operator guide, index). Secret/staging audit **PASS**. Committed **`f35d23f`** `docs: certify StockBallDB phase 9`. **PHASE 9 = COMPLETE**; **NEXT = Phase 10 â Operational Workflow**.

## Step 51 â Push Phase 9 to GitHub
...
## Step 52 â Phase 10A operational workflow audit & contract
...
## Step 52 â CLI entry point inventory
...
## Step 54 â Commit Phase 10A operational contract
...
## Step 55 â Phase 10B unified operational update
...
## Step 56 â Commit Phase 10B implementation
...
## Step 57 â Phase 10C operational certification
...
## Step 58 â Document Phase 10 certification

Updated contract Â§27, operator guide, `phase_history.md`. Committed **`3e5f797`** `docs: certify StockBallDB phase 10`.

## Step 59 â Push Phase 10 to GitHub
...
## Step 60 â Phase 11A Explorer audit & contract
...
## Step 61 â Commit Phase 11A explorer contract
...
## Step 62 â Phase 11B StockBallDB Explorer implementation

Implemented read-only Streamlit Explorer (`python -m stockballdb.explorer`): isolated read-only DB engine, seven-table registry/query layer, artifacts/formatting/definitions, six service modules, six Streamlit pages with sidebar Refresh and CSV export, and `tests/test_explorer.py` (47 tests). Added `streamlit>=1.40,<2`, `explorer_database_url()` config helper, and `docs/StockBallDB_explorer.md`. Updated contract Â§11B record and index. pytest **189/189 PASS**; validate_v1 **PASS**; health **HEALTHY**; fingerprint unchanged; Streamlit launch verified on localhost:8501. **PHASE 11B = IMPLEMENTED**; **PHASE 11C = NOT STARTED**.

## Step 63 â Commit and push Phase 11B Explorer
...
## Step 66 â Fix Explorer live Database Status caching
...
## Step 69 â Add auto-commit Cursor rule
...
## Step 70 â Fix Explorer date_input Streamlit Â±10y bounds
...
## Step 71 â Fix Coverage Explorer health API adapter (11C)
...
## Step 72 â Phase 11C certification closeout

Formal closeout audit (2026-09-10): pytest **211 passed, 2 skipped**; validate_v1 **PASS**; health **HEALTHY** (3 INFO freshness/gap findings); primary fingerprint **MATCH** `sha256:5055ae10f216ff33141eb137fc396ce29481e896c3ea828983e4ca8b12291aca`; latest manifest `20260831T145447-9be32508` snapshot verify checked=91 missing=0 corrupt=0 **PASS**; Explorer package read-only audit PASS (no DML/DDL/update/rebuild/SQL console). Manual six-surface certification accepted. **PHASE 11C = CERTIFIED**; **PHASE 11 = COMPLETE**. No Phase 12 defined in repo â next action is planning/design.

## Step 73 â Push Phase 11C commits to GitHub
...
## Step 74 â Phase 11D Explorer desktop UI refresh planning (no implementation)
...
## Step 75 â Implement Phase 11D Explorer desktop UI refresh

Locked `docs/StockBallDB_phase11d_explorer_desktop_ui_contract.md`. Added `.streamlit/config.toml`, `explorer/ui/` helpers (CSS, chrome header, components, dataframe styling), wide top-nav shell in `app.py`, and recomposed all six views for desktop density. Preserved read-only services/queries/health. pytest **213 passed, 2 skipped**; validate_v1 **PASS**; health **HEALTHY**; fingerprint **MATCH** `sha256:5055ae10...`. Status: **PHASE 11D IMPLEMENTED â MANUAL CERTIFICATION PENDING**.

## Step 76 â Phase 11D visual polish Pass 2
...
## Step 77 â Phase 11D Pass 3 shell/nav fidelity
...
## Step 78 â Phase 11D Pass 4 flush top bar
...
## Step 79 â Fix Data Explorer decimal-ratio percent display
...
## Step 80 â Round adj OHLC to 2 decimals on screen

Display-only 2 d.p. for `adj_open` / `adj_high` / `adj_low` / `adj_close` in Explorer formatting (e.g. `584.1333193` â `584.13`). CSV/DB unchanged. Explorer tests **70 passed**.

## Step 81 â Push Phase 11D commits to GitHub

Pushed local `main` to `origin/main` (`e6d4013..d3350e1`): Phase 11D Passes 1â4 UI refresh, percent display fix, adj OHLC 2 d.p. display. Branch up to date with origin.

## Step 82 â Report primary PostgreSQL sizes (read-only)
...
## Step 83 â Fix Streamlit use_container_width deprecations
...
## Step 84 â Documentation consolidation audit (Step 1 only)

Inspected all 20 `docs/StockBallDB_*.md` files plus README references against the consolidation table. Produced a migration report (source â destination, preserve vs drop, contradictions, out-of-table `phase11d`). No documentation files rewritten or deleted; awaiting approval before Step 2 consolidate.
...
## Step 87 — Canonical validation permanent doc
